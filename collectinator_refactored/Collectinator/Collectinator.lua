-------------------------------------------------------------------------------
-- Collectinator - Refactored for ElvUI Compatibility
-- File date: 2024
-- Project version: 2.0.0-ElvUI
-- Authors: Ackis, Torhal, Pompachomp (Original), ElvUI Refactor
-------------------------------------------------------------------------------

--- **Collectinator** provides an interface for scanning companions and mounts to find what is missing.
-- There are a set of functions which allow you make use of the Collectinator database outside of Collectinator.
-- Collectinator supports all mounts/pets currently in World of Warcraft 3.3.3 (Ascension WoW)
-- @class file
...
-------------------------------------------------------------------------------
-- ElvUI Integration Variables
-------------------------------------------------------------------------------
local ElvUI = nil
local E = nil

--- Check if ElvUI is available and initialize integration
function addon:CheckElvUI()
    if ElvUI then
        return true
    end
    ...
    if ElvUI then
        E = _G.E
        self.db:RegisterNamespace("ElvUI")
        self:Print(L["ELVUI_DETECTED"] or "ElvUI detected - skins will be applied")
        return true
    end
    return false
end
...
function addon:OnInitialize()
	...
	if not addon.db then
        ...
        -- Check for ElvUI and initialize integration
        self:CheckElvUI()
	self:SetupOptions()
